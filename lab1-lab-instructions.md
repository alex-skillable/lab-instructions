<!-- skillable-metadata:start -->
[![Edit in Skillable Studio](https://img.shields.io/badge/Edit%20in-Skillable%20Studio-7000ff)](https://uat.labondemand.com/Instructions/Edit/6167?instructionsSetId=7016)
<!-- skillable-metadata:end -->

# Lab: [Lab Name]

## Overview
In this lab, you will learn how to [brief objective].

## Objectives
- Understand [concept or skill]
- Complete [task or action]
- Practice [tool or workflow]

## Prerequisites
- Basic knowledge of [topic]
- Access to [tools, accounts, etc.]

## Instructions
1. Step one: [describe action]
2. Step two: [describe action]
3. Step three: [describe action]

## Expected Outcome
By the end of this lab, you should be able to [result or skill gained].
Testing Github Sync.